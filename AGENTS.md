...
- **Authentication**: Firebase-based auth via `kreait/laravel-firebase`. `VerifyFirebaseIdToken` validates ID tokens. `POST /api/auth/sync` upserts local users. **`POST /api/admin/access-requests`** (Firebase-authenticated) queues admin access; reviewer receives **signed** web links (`GET /admin/access-requests/{id}/approve|reject`) that set `role` / `admin_access_status`. Middleware **`admin.approved`** requires `role === admin` and `admin_access_status === approved` (used on a testing-only gate route in `testing`; apply to admin API routes when ready). Reviewer inbox: `ADMIN_ACCESS_REVIEW_EMAIL` / `config('admin_access.review_email')`.
