...
test('resolves the Noop provider from a slug', function () {
    $resolver = app(PushProviderResolver::class);

    $provider = $resolver->resolve('noop');

    expect($provider)->toBeInstanceOf(PushProviderContract::class)
        ->and($provider)->toBeInstanceOf(NoopPushProvider::class);
});
...
test('throws for an unsupported provider slug (apns future)', function () {
...
test('registers the Noop provider as a singleton', function () {
    $first = app(NoopPushProvider::class);
